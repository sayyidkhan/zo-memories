...
export function Input({ className, ...props }: InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      className={cn(
        "h-12 w-full rounded-2xl border border-[#d8cdbc] bg-[#fffdf8] px-4 text-[15px] text-[#26372f] outline-none transition focus:border-[#728578] focus:ring-4 focus:ring-[#728578]/10 placeholder:text-[#9b9387]",
        className,
      )}
      {...props}
    />
...
export function Field({ label, children, hint }: { label: string; children: ReactNode; hint?: string | undefined }) {
  return (
    <label className="grid gap-2 text-sm font-semibold text-[#3a493f]">
      {label}
      {children}
...
export function Modal({
  ...
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-50 grid place-items-end bg-[#17231e]/45 p-0 backdrop-blur-[3px] sm:place-items-center sm:p-6" role="presentation" onMouseDown={onClose}>
      <section
        role="dialog"
        aria-modal="true"
        aria-labelledby="modal-title"
        className={cn(
          "max-h-[92vh] w-full overflow-y-auto rounded-t-[30px] bg-[#fffaf2] p-6 shadow-2xl animate-in sm:rounded-[30px] sm:p-8",
      ...
      >
        <div className="mb-6 flex items-start justify-between gap-4">
          <div>
            <h2 id="modal-title" className="font-display text-3xl leading-tight text-[#26372f]">{title}</h2>
