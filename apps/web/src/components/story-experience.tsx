    ...
    <>
      <article data-editing={editing || undefined} data-reveal={reveal || undefined} onScroll={reviewProgress} className="fixed inset-0 z-[60] overflow-y-auto bg-[#f3eadc] text-[#23372d]">
        <div className="fixed right-3 top-[max(.75rem,env(safe-area-inset-top))] z-30 flex items-center gap-2 sm:right-4 sm:top-4">
          {editing ? <><span className={cn("inline-flex size-11 items-center justify-center rounded-full bg-[#183128]/88 text-[#fff9ee] shadow-xl backdrop-blur-md sm:hidden", saveCanvas.isError && "bg-[#8a372b]")} aria-label={saveLabel}>{saveCanvas.isPending ? <Spinner /> : <Cloud className="size-4" />}</span><span className={cn("hidden h-11 items-center gap-2 rounded-full bg-[#183128]/88 px-4 text-xs font-bold text-[#fff9ee] shadow-xl backdrop-blur-md sm:inline-flex", saveCanvas.isError && "bg-[#8a372b]")}>{saveCanvas.isPending ? <Spinner /> : <Cloud className="size-4" />}{saveLabel}</span><button onClick={() => setHistoryOpen(true)} className="inline-flex size-11 items-center justify-center gap-2 rounded-full bg-[#fff9ee]/94 text-sm font-bold text-[#26372f] shadow-xl backdrop-blur-md sm:h-12 sm:w-auto sm:px-5" aria-label="Version history"><History className="size-4" /><span className="hidden sm:inline">Versions</span></button><button onClick={finishEditing} disabled={!canvasValid || saveCanvas.isPending} className="inline-flex h-11 items-center justify-center gap-2 rounded-full bg-[#f0c681] px-4 text-sm font-bold text-[#26372f] shadow-xl transition hover:bg-[#f6d795] disabled:cursor-not-allowed disabled:opacity-60 sm:h-12 sm:px-5"><Check className="size-4" />Done</button></> : <>
            {canEdit ? <button onClick={() => setEditing(true)} className="inline-flex size-11 items-center justify-center gap-2 rounded-full bg-[#fff9ee]/90 text-sm font-bold text-[#26372f] shadow-xl backdrop-blur-md transition hover:scale-[1.03] sm:h-12 sm:w-auto sm:px-5" aria-label="Edit story"><PencilLine className="size-4" /><span className="hidden sm:inline">Edit</span></button> : null}
            <div className="relative">
              <button onClick={openShare} className={cn("inline-flex size-11 items-center justify-center gap-2 rounded-full bg-[#f0c681] text-sm font-bold text-[#26372f] shadow-xl backdrop-blur-md transition hover:scale-[1.03] hover:bg-[#f6d795] sm:h-12 sm:w-auto sm:px-5", shareCue && "story-share-cue")} aria-label="Share story">
                <Share2 className="size-4" />
                <span className="hidden sm:inline">Share story</span>
              </button>
              {shareCue ? <button type="button" onClick={openShare} className="story-share-callout absolute right-0 top-[calc(100%+.8rem)] w-[13.5rem] rounded-[20px] border border-[#f0c681]/70 bg-[#183128]/96 p-3.5 text-left text-[#fff9ee] shadow-[0_20px_55px_rgba(4,15,10,.38)] backdrop-blur-md sm:w-[15rem] sm:p-4" aria-label="Your story is ready. Choose an image or video to share.">
                <span className="flex items-start gap-3">
                  <span className="grid size-9 shrink-0 place-items-center rounded-full bg-[#f0c681] text-[#26372f]"><Sparkles className="size-4" /></span>
                  <span>
                    <strong className="block text-sm leading-5">Your story is ready</strong>
                    <span className="mt-0.5 block text-xs leading-5 text-[#e9ddc7]">Choose an image or video to share.</span>
                  </span>
                </span>
              </button> : null}
            </div>
